...
from ..tree_utils import BranchFactory, GradHess, GradHessStats


class Splitter(Estimator, abc.ABC):
    ...
    @abc.abstractmethod
    def update(self, att_val, target_val, sample_weight: float):
    ...
    @abc.abstractmethod
    def cond_proba(self, att_val, target_val) -> float:
...
class Quantizer(Estimator, abc.ABC):
