...
class GLM:
    ...
    def _raw_dot_one(self, x: dict) -> float:
        return self._weights @ VectorDict(x) + self.intercept
    ...
    def _eval_gradient_one(self, x: dict, y: float, w: float) -> typing.Tuple[dict, float]:
        ...
        loss_gradient *= w
        loss_gradient = float(
            clamp(loss_gradient, -self.clip_gradient, self.clip_gradient)
        )

        if self.l2:
            return (
                loss_gradient * VectorDict(x) + self.l2 * self._weights,
                loss_gradient,
            )

        return (loss_gradient * VectorDict(x), loss_gradient)
