...
from ..utils import inspect, math
...
class ClassificationMetric(Metric):
    ...
    def works_with(self, model) -> bool:
        return inspect.isclassifier(model)
    ...
    def _clamp_proba(p):
        ...
        return math.clamp(p, minimum=1e-15, maximum=1 - 1e-15)
...
class MultiClassMetric(ClassificationMetric):
    ...
    def works_with(self, model) -> bool:
        return inspect.isclassifier(model) or inspect.isclusterer(model)
...
class RegressionMetric(Metric):
    ...
    def works_with(self, model) -> bool:
        return inspect.isregressor(model)
...
class ClusteringMetric(Base, abc.ABC):
    """
    ...
    def works_with(self, model) -> bool:
        """Indicates whether or not a metric can work with a given model."""
        return inspect.isclusterer(model)
